...
Como detalle, en este caso voy a usar una herramienta de torch para crear el dataset. 
Las dimensiones van a ser [Batch_size, Longitud de seq, input features]
...
"""
Puedes ir abajo directamente y no te preocupes mucho de esto
"""
...
class LSTMModel_parallel(nn.Module):
    ...
    def forward(self, x):
        ...
        # Salida del LSTM
        lstm_out, _ = ...  # lstm_out tiene la forma (batch_size, seq_lenght, hidden_size)
        # Tomamos solo la última salida del LSTM para cada secuencia
        lstm_out = ... # Ahora lstm_out tiene la forma (batch_size, hidden_size)

        # Salida de la capa lineal
        #opcion 1: solo el ultimo dia de la sequencia
        mlp_output = ...  # Usamos solo el último elemento de la secuencia
        ...
        return final_output
"""
Cuando todo funcione bien, prueba a añadir otras modificaciones al modelo
